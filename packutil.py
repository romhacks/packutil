...
if not os.path.exists("textures"): #checks if textures exists- if not download and extract them
    sg.popup(
    ...
    with ZipFile("textures/textures.zip", "r") as zipFile:
        ...
        for file in zipFile.namelist(): #namelist is a list of every file in the zip
            updateProg += 1
            if updateProg == 500: #only update meter every 500 files because there's 40k of them
